                            ...
                            <span class="abs-basketitem-small-price hidden-sm-up">
                                [{if !$basketitem->isBundle()}]
                                    ...
                                    [{if $dRegUnitPrice && $dUnitPrice && $dRegUnitPrice->getPrice() > $dUnitPrice->getPrice()}]
                                        <s class="text-danger abs-inlinelineblock">[{oxprice price=$basketitem->getRegularUnitPrice() currency=$currency}]</s>
                                    [{/if}]
                                [{/if}]
                                [{if $basketitem->getUnitPrice()}]
                                    <span class="abs-inlinelineblock">[{oxprice price=$basketitem->getUnitPrice() currency=$currency}]</span>
                                [{/if}]
                            </span>
                        </div>
                    [{/block}]
                                ...
                                <div class="col-xs-12 col-md-6">
                                    <div>
                                        ...
                                        [{if $basketitem->isSkipDiscount()}]
                                            <sup><a rel="nofollow" href="#noteWithSkippedDiscount">[{$oMarkGenerator->getMark('skippedDiscount')}]</a></sup>
                                        [{/if}]
                                        [{if $oViewConf->getActiveClassName() == 'order' && $oViewConf->isFunctionalityEnabled('blEnableIntangibleProdAgreement')}]
                                            [{if $oArticle->hasDownloadableAgreement()}]
                                                <sup><a rel="nofollow" href="#noteForDownloadableArticles">[{$oMarkGenerator->getMark('downloadable')}]</a></sup>
                                            [{/if}]
                                            [{if $oArticle->hasIntangibleAgreement()}]
                                                <sup><a rel="nofollow" href="#noteForIntangibleArticles">[{$oMarkGenerator->getMark('intangible')}]</a></sup>
                                            [{/if}]
                                        [{/if}]
                                    </div>
                                    <div>
                                    ...
                                    [{block name="checkout_basketcontents_basketitem_quantity"}]
                                        ...
                                        [{if $editable}]
                                            ...
                                            [{/if}]
                                        [{else}]
                                            [{oxmultilang ident="COUNT" suffix="COLON"}] [{$basketitem->getAmount()}]
                                        [{/if}]
            ...
            <div class="row text-xs-right">
                [{if !( $oxcmp_basket->getDiscounts() || ($oViewConf->getShowVouchers() && $oxcmp_basket->getVoucherDiscValue())  )}]
                    [{block name="checkout_basketcontents_nodiscounttotalnet"}]
                        ...
                        <div class="col-xs-4 col-sm-3 col-sm-offset-4 col-lg-offset-0 col-lg-2">
                ...
                [{block name="checkout_basketcontents_paymentcosts"}]
                    ...
                    [{if $paymentCost && $paymentCost->getPrice()}]
                        [{if $oViewConf->isFunctionalityEnabled('blShowVATForPayCharge')}]
                            <div class="col-xs-8 col-sm-5 col-sm-offset-0 col-lg-3 col-lg-offset-7 text-xs-left">
                                [{if $paymentCost->getPrice() >= 0}]
                                    [{oxmultilang ident="SURCHARGE"}]
                                [{else}]
                                    [{oxmultilang ident="DEDUCTION"}]
                                [{/if}]
                ...
                [{block name="checkout_basketcontents_ts"}]
                    ...
                    [{if $trustedShopProtectionCost && $trustedShopProtectionCost->getPrice() > 0}]
                        ...
                        [{else}]
                            ...
                            <div class="col-xs-4 col-sm-3 col-sm-offset-4 col-lg-offset-0 col-lg-2">
                                [{oxprice price=$trustedShopProtectionCost->getBruttoPrice() currency=$currency}]
                            </div>
                        [{/if}]
                    [{/if}]
                [{/block}]
                ...
                [{block name="checkout_basketcontents_grandtotal"}]
                    ...
                    <div class="col-xs-4 col-sm-3 col-sm-offset-4 col-lg-offset-0 col-lg-2">
                        <strong>[{oxprice price=$oxcmp_basket->getPrice() currency=$currency}]</strong>
                    </div>
                [{/block}]

                [{if $oxcmp_basket->hasSkipedDiscount()}]
                    <div class="col-xs-8 col-sm-5 col-sm-offset-0 col-lg-3 col-lg-offset-7 text-xs-left">
                        <span id="noteWithSkippedDiscount">
                            [{$oMarkGenerator->getMark('skippedDiscount')}]
                        </span>
                        <span> [{oxmultilang ident="MESSAGE_COUPON_NOT_APPLIED_FOR_ARTICLES"}]</span>
                    </div>
