[{capture append="oxidBlock_content"}]
    ...
    [{block name="checkout_thankyou_main"}]
        ...
        <h3 class="h1">[{oxmultilang ident="THANK_YOU"}]</h3>
        ...
        [{block name="checkout_thankyou_info"}]
            <div class="m-y-1">
                ...
                <p>[{oxmultilang ident="REGISTERED_YOUR_ORDER" args=$order->oxorder__oxordernr->value}]</p>
            </div>
            <div class="m-y-1">
        ...
        [{block name="checkout_thankyou_proceed"}]
            ...
            <a id="backToShop" rel="nofollow" href="[{oxgetseourl ident=$oViewConf->getHomeLink()}]" title="[{oxmultilang ident="BACK_TO_START_PAGE"}]">[{oxmultilang ident="BACK_TO_START_PAGE"}]</a>
            [{if $oxcmp_user->oxuser__oxpassword->value}]
                [{oxmultilang ident="OR"}]
                <a id="orderHistory" rel="nofollow" href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account_order"}]" title="[{oxmultilang ident="CHECK_YOUR_ORDER_HISTORY"}]">[{oxmultilang ident="CHECK_YOUR_ORDER_HISTORY"}]</a>.
            [{/if}]
        [{/block}]
        ...
        [{block name="checkout_thankyou_ts"}]
            [{if $oViewConf->showTs("THANKYOU") && $oViewConf->getTsId()}]
                <div class="m-y-1">
                    ...
                    <h3 class="h3">[{oxmultilang ident="TRUSTED_SHOPS_CUSTOMER_RATINGS"}]</h3>
        ...
        [{block name="checkout_thankyou_partners"}]
            [{if $oView->getTrustedShopId()}]
                <div class="m-y-1">
                    <h3 class="h5">[{oxmultilang ident="TRUSTED_SHOP_BUYER_PROTECTION"}]</h3>
                    ...
                    <form id="formTsShops" name="formTShops" method="post" action="https://www.trustedshops.com/shop/protection.php" target="_blank">
                        ...
                        <div>
                            <p>[{oxmultilang ident="TRUSTED_SHOP_REGISTRATION_MESSAGE"}]</p>
                            <p><button class="btn btn-primary" type="submit" id="btnProtect" name="btnProtect" value="[{oxmultilang ident="TRUSTED_SHOP_REGISTRATION"}]">[{oxmultilang ident="TRUSTED_SHOP_REGISTRATION"}]</button></p>
                        </div>
                    </form>
                </div>
            [{/if}]
