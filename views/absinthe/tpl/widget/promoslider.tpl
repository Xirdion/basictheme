...
<div id="abs-promo-slider" class="container m-t-2 hidden-sm-down">
    <div id="absjs-slick-promo">
        [{foreach name=sliderpics from=$oBanners item=oBanner key=iIndex}]
        <div>
            [{if $sBannerLink }]
                <a href="[{$sBannerLink}]" title="[{$oBanner->oxactions__oxtitle->value}]">
            [{/if}]
            <img data-lazy="[{$oBanner->getBannerPictureUrl()}]">
            [{if $sBannerLink }]
                </a>
            [{/if}]
        </div>
